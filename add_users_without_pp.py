...
def check_pp(pp_id, pp_count):  # Данная функция ищет количество совпадений среди групп пользователя
    pp_id = open("PP.txt", "r")  # В файле содержится список id "сомнительных" пабликов
    pp_id = pp_id.read()
    pp_id = [int(x) for x in pp_id.split(',')]
    groups_response = vk.groups.get(user_id=pp_id)
    return len(set(groups_response['items']).intersection(pp_id)) > pp_count
